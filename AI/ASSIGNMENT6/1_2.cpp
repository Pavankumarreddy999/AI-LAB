...
struct Node{
    ...
    int f;  
    vector<string> path;
    Node(string c,int gCost,int hCost,vector<string> p){
        city=c;
        g=gCost;
        h=hCost;
        f=g+h;
...
void aStarSearch(vector<vector<int>>&graph,vector<string>&cities,vector<int>&heuristic,string start,string goal){
    int n=cities.size();
    ...
    pq.push(Node(start,0,heuristic[startIndex],{start}));
    int pathsExplored=0;
    while(!pq.empty()){
        Node current=pq.top();
        pq.pop();
        ...
        cout<<" | g(n): "<<current.g <<" | h(n): "<<current.h<<" | f(n): "<<current.f<<"\n";
        ...
        for(int v=0;v<n;v++){
            if(graph[u][v]!=1e9 && !visited[v]){
                int newG = current.g + graph[u][v];
                vector<string> newPath=current.path;
                newPath.push_back(cities[v]);
                pq.push(Node(cities[v],newG,heuristic[v],newPath));
            }
        }
    }
...
int main(){
    vector<string> cities={
        "Chicago","Indianapolis","Cleveland","Detroit","Columbus",
        "Pittsburgh","Baltimore","Philadelphia","Buffalo",
        "New York","Providence","Boston","Portland","Syracuse"
    };
    ...
    cout << "--------A* SEARCH--------" << "\n";
    aStarSearch(adjMatrix,cities,heuristic,"Chicago","Boston");
