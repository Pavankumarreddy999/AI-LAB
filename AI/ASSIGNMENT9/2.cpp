...
//  MINIMAX + ALPHA-BETA
int minimaxAB(bool isMax, int alpha, int beta, string prefix) {
    nodesVisited++;
        ...
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                if(board[i][j]=='_'){
                    ...
                    best = max(best, minimaxAB(false, alpha, beta, prefix + "│   "));
        ...
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                if(board[i][j]=='_'){
                    ...
                    best = min(best, minimaxAB(true, alpha, beta, prefix + "│   "));
    ...
    for(int i=0;i<3;i++){
        for(int j=0;j<3;j++){
            if(board[i][j]=='_'){
                ...
                int moveVal = minimaxAB(false, -1000, 1000, "│   ");
